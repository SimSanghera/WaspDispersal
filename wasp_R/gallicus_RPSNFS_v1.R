...
fieldArena <- 
  as.matrix(read.delim2("wasp_data_output/gallicus_RPSNFS/RSF_arena_1.txt"),
                        sep = "\t")
...
for (i in 1:length(gallicusOutList)) {
  write.table(gallicusOutList[[i]],
              paste0("wasp_data_output/gallicus_RPSNFS/gallicus_",
                     "RSF_",
...
write.table(fieldArena,
            "wasp_data_output/gallicus_RPSNFS/RSF_arena_10.txt",
            row.names = FALSE,
            sep = "\t")

write.table(patchCoords, 
            "wasp_data_output/gallicus_RPSNFS/patchCoords_RSF_10.txt",
            row.names = FALSE)

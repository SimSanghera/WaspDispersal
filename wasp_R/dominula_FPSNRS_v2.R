...
#- Fixed Patch Size, Single Nest, Random Start
#   
#   In this set of simulations, number of wasps will increase by 10,
#     from 50-200.
#   All wasps will originate from a single nest.
#   Starting coordinates generated in first simulation will be used for every nest.
#   A single habitat will be generated & used for every simulation.
...
write.table(fieldArena,
            "wasp_data_output/dominula_FPSNRS/FSR_arena_1.txt",
            row.names = FALSE,
            sep = "\t")
...
fieldArena <- as.matrix(read.delim2("wasp_data_output/dominula_FPSNRS/FSR_arena_1.txt"),
                        sep = "\t")
...
for (d in seq(50, 200, 10)) { # for each nest
  ...
  domN <- d           # number of wasps in nest
  fieldArena <- a     # reset habitat
...
for (i in 1:length(dominulaOutlist)) {
  write.table(dominulaOutlist[[i]],
              paste0("wasp_data_output/dominula_FPSNRS/dominula_",
                     "RSF_",

...
write.table(patchCoords, 
            "wasp_data_output/patchCoords_RSF_9.txt",
            row.names = FALSE)
...
write.table(patchCoords, 
            "wasp_data_output/gallicus_patchCoords_RSR_10.txt",
            row.names = FALSE)
...
nestList <- list()
nestPath <- list()
tracking <- data.frame(g = NA, n = NA, w = NA)
...
#----- 5. Fixed Patch Size, Single Nest, Fixed Starting Coord
#
# In this set of simulations, the number of wasps will increase
#   by 10, from 50-200.
# All wasps will originate from a single nest.
...
a <- createHabitatFixed(1000, 1000, 10, 0.3)
habitatV_NV <- table(fieldArena)
patchCoords <- which(fieldArena == 0, arr.ind = T)
plot(which(fieldArena == 0, arr.ind = T))
...
write.table(patchCoords, 
            "wasp_data_output/patchCoords_FSF.txt",
            row.names = FALSE)

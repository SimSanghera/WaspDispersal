...
for (i in 1:length(dominulaOutlist)) {
  write.table(dominulaOutlist[[i]],
              paste0("wasp_data_output/dominula_RPSNRS/dominula_",
                     "RSR_",
...
write.table(fieldArena,
            "wasp_data_output/dominula_RPSNRS/RSR_arena_10.txt",
            row.names = FALSE,
            sep = "\t")

write.table(patchCoords, 
            "wasp_data_output/dominula_RPSNRS/patchCoords_RSR_10.txt",
            row.names = FALSE)

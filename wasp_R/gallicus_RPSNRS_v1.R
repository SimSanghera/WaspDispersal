...
a <- fieldArena # this is a cheap hack to store fieldArena 
#  for repeated use, bc for some reason "a" &
#  and fieldArena weren't the same.
...
for (i in 1:length(gallicusOutList)) {
  write.table(gallicusOutList[[i]],
              paste0("wasp_data_output/gallicus_RPSNRS/gallicus_",
                     "RSR_",
...
write.table(fieldArena,
            "wasp_data_output/gallicus_RPSNRS/RSR_arena_10.txt",
            row.names = FALSE,
            sep = "\t")

write.table(patchCoords, 
            "wasp_data_output/gallicus_RPSNRS/patchCoords_RSR_10.txt",
            row.names = FALSE)

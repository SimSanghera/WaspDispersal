  ...
  for (w in 1:domN) { # for each wasp
    ...
    while (alive == 1 & nested == 0) { # until wasp nests
      ...
      if (fieldArena[currentX, currentY] == 0) {
        ...
        # 2. if there is a single wasp in the nest, new wasp 
        #   may usurp, join, or move on
        # However, join has an optimal threshold
        
        if (fieldArena[currentX, currentY] == 1) { # cell occupied
          
          # a. check if usurps
          # set initial usurp value as 0
          usurp <- 0
          
          # then check if does usurp
          ...
          if (usurp == 0) {
            # b. if the wasp doesn't usurp, it could possibly join
          ...
          # end cell only occupied by 1 wasp
          
        } else {
          
          # 3. if there are 2 + wasps, the arriving wasp can only join
          #   or move on
          if (fieldArena[currentX, currentY] >= 2) {
            
            # check if number of wasps is more/less than optimum to set
            #   probabilities of joining
            ...
            # now check to see if wasp joins
            probJoin <- sample(c(0, 1), prob = c((1 - probJoin), probJoin),
                             1, replace = T)
            
            if (pdJoin == 1) { # wasp joins
            ...
            } else { # wasp doesn't join
              
              nested <- 0
              coNest <- 0
              usurp <- 0
              
            } # end of joining multiples
            
          } else {
            
            if (fieldArena[currentX, currentY] == -1) { # nonviable cell
      ...
      dom <- cbind(w, newPath[1], startX, startY,
...
for (i in 1:length(dominulaOutlist)) {
  write.table(dominulaOutlist[[i]],
              paste0("wasp_data_output/domRSF/dominula_",
                     "RSF_",

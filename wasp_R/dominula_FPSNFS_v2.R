...
write.table(fieldArena,
            "wasp_data_output/dominula_FPSNFS/FSF_arena_1.txt",
            row.names = FALSE,
            sep = "\t")

# read field arena when needed
fieldArena <- as.matrix(read.delim2("wasp_data_output/dominula_FPSNFS/FSF_arena_1.txt"),
                        sep = "\t")
...
for (i in 1:length(dominulaOutlist)) {
  write.table(dominulaOutlist[[i]],
              paste0("wasp_data_output/dominula_FPSNFS/dominula_",
                     "RSF_",

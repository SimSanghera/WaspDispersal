...
a <- fieldArena # a cheap hack to store the fieldArena for repeated
                # use, because for some reason a & fieldArena weren't 
                # the same
...
for (g in seq(50, 200, 10)) {
  ...
  for (w in 1:gallN) {
    ...
    while (alive == 1 & nested = 0) {
      ...
      # check if wasp is alive and if alive - run loop
      ...
      if (alive == 1) {
        ...
        if (fieldArena[currentX, currentY] == 0) {
          ...
          if (fieldArena[currentX, currentY] == 1) {
            ...
            # check if usurps, & if usurps, then nest
        ...
        # removed check non-viable - as might be redundant
        
      } #  end if alive
      
      # store results
      gall <- rbind(w, s, startX, startY,
                    alive, nested, usurp,
                    currentX, currentY)

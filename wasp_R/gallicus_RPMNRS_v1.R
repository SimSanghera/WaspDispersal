...
# load field arena
fieldArena <- 
  as.matrix(read.delim2("wasp_data_output/gallicus_RPMNRS/RMR_arena_n10_1.txt"),
            sep = "\t")
...
for (n in 1:10) { # for each nest - just change number of nests
...
#nest1 <- as.data.frame(do.call("rbind", totalGallicus[[1]]))
...
write.table(fieldArena,
            "wasp_data_output/gallicus_RPMNRS/RMR_arena_n10_1.txt",
            row.names = FALSE,
            sep = "\t")

write.table(patchCoords, 
            "wasp_data_output/gallicus_RPMNRS/patchCoords_RMR_n10_1.txt",
            row.names = FALSE)

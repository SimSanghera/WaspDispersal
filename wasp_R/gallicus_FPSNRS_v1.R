...
#- Random Patch Size, Single Nest, Random Start
#   
#   In this set of simulations, number of wasps will increase by 10,
#     from 50-200.
#   All wasps will originate from a single nest.
#   Starting coordinates will be randomly generated for every nest.
#   Only 1 habitat will be generated & used for each simulation.
...
write.table(fieldArena,
            "wasp_data_output/gallicus_FPSNRS/FSR_arena_1.txt",
            row.names = FALSE,
            sep = "\t")

write.table(patchCoords, 
            "wasp_data_output/gallicus_FPSNRS/patchCoords_FSR_1.txt",
            row.names = FALSE)

# read field arena when needed
fieldArena <- as.matrix(read.delim2("wasp_data_output/gallicus_FPSNRS/FSR_arena_1.txt"),
                        sep = "\t")
...
for (g in seq(50, 200, 10)) {
  ...
  fieldArena <- a     # reset field arena to 
  ...
  for (w in 1:gallN) {
    ...
    while (alive == 1 & nested == 0) {
      ...
      # check if wasp is alive. IF ALIVE, run rest of loop
...
for (i in 1:length(gallicusOutList)) {
  write.table(gallicusOutList[[i]],
              paste0("wasp_data_output/gallicus_FPSNRS/gallicus_",
                     "FSR_",
